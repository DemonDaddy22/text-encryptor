...
const Generate = () => {
  ...
  const handleInputTextChange = useCallback(
    (value) => setInputTextVal(value),
    []
  );

  const handleInputSecretKeyChange = useCallback(
    (value) => setInputSecretKey(value),
    []
  );
  ...
  const handleEncryption = useCallback(() => {
    const message =
      isEmptyString(inputTextVal) ||
      isEmptyString(inputSecretKey) ||
      isEmptyString(validity)
        ? "Missing either Text to Encrypt or Secret Key or Validity Duration!"
        : "";
    const severity =
      (isEmptyString(inputTextVal) || isEmptyString(inputSecretKey)) &&
      SEVERITY.INFO;
  ...
  useEffect(
    () => fetchResult && sendNewMessageRequest(),
    [fetchResult, sendNewMessageRequest]
  );
          ...
          {isEmptyString(url) ? (
            <div className={classes.generatorContainer}>
              ...
              <div className={classes.container}>
                <Input
                  inputVal={inputSecretKey}
                  handleInputChange={handleInputSecretKeyChange}
                  placeholderValue="Enter Secret Key"
                  style={{ flex: 1 }}
                  rows={1}
                />
                <Select
            ...
            <div className={classes.generatorContainer}>
              <div className={classes.urlContent}>
                <div className={classes.urlText} onClick={openInNewTab}>
                  {url}
                </div>
